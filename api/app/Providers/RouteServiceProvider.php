...
class RouteServiceProvider extends ServiceProvider
{
    ...
    {
        // Общий лимит для API: 180 запросов/мин на пользователя (3/сек)
        RateLimiter::for('api', function (Request $request) {
            return Limit::perMinute(180)->by(optional($request->user())->id ?: $request->ip());
        });

        // Логин: 5 попыток/мин с IP
        RateLimiter::for('login', function (Request $request) {
            return Limit::perMinute(5)->by($request->ip());
        });

        // Критичные операции (создание/удаление): 30 запросов/мин
        RateLimiter::for('mutations', function (Request $request) {
            return Limit::perMinute(30)->by(optional($request->user())->id ?: $request->ip());
        });

        // Админ API: 60 запросов/мин
        RateLimiter::for('admin', function (Request $request) {
            return Limit::perMinute(60)->by(optional($request->user())->id ?: $request->ip());
        });

        // Broadcasting auth: 20 запросов/мин
